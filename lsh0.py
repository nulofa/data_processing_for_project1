...
if __name__ == '__main__':
    ...
    bits = 10    # number of bits (planes) per signature
    ...
    for di in range(len(data)):
        ind = data[di].argsort()
        data2[di][ind] = range(1,dim+1)


    # print(data2[0])
    table_num = 40

    tables, planes = lsh_tables(data2, bits, dim, table_num)
    ...
    while string != 'end':
        ...
        for qn in range(query_num):
            ...
            qq = np.random.randn(dim)
            query_q = qq.copy()
            query_q[qq.argsort()] = range(1, dim + 1)
            ...
            for i in range(table_num):
                ...
                # print('bit4q= ',bit4q)
                for b in range(len(bit4q)//len(bit4q)):
                    if bit4q[b] == '0':
                ...
                for sig4q in bins:
                    ...
                    max_cos = cos_sim(query_q, data2[ind])

                    res = ind

                    for p in tables[i][sig4q]:
                        cosine = cos_sim(data2[p], query_q)
                        if cosine > max_cos:
                            max_cos = cosine
                            res = p
            ...
            s2 = time.clock()
            res2, cos2 = linear_search(data2, query_q)
            e2 = time.clock()
